 ...
 *
 */
Inclusion.include = function (objects, include, options, cb, includefilter, multiPointer) {
  ...
  async.each(include, function(item, callback) {
    processIncludeItem(objects, item, options, callback, includefilter, multiPointer);
  }, function(err) {
    cb && cb(err, objects);
  });

  function processIncludeItem(objs, include, options, cb, includefilter, multiPointer) {
    ...
    if (relation.multiple) {
      ...
      return includeHasMany(cb,includefilter,multiPointer);
    }
    else {
      if (polymorphic) {
      ...
      //hasOne or belongsTo
      return includeOneToOne(cb, includefilter, multiPointer);
    }
     ...
     * @param callback
     */
    function includeHasMany(callback, includefilter, multiPointer) {
      var sourceIds = [];
      //Map for Indexing objects by their id for faster retrieval
      var objIdMap = {};
      for (var i = 0; i < objs.length; i++) {
      ...
      var targetFilter = includefilter[relation.modelTo.modelName] || {};
      filter.where[relation.keyTo] = {
        inq: uniq(sourceIds)
      };
      mergeQuery(filter,targetFilter);
      var aggregate = [];
      aggregate.push({'where': filter.where});
      if (filter.order) {
      ...
      relation.applyScope(null, filter);
      relation.modelTo.aggregate(aggregate, targetFetchHandler);
      /**
      ...
      function targetFetchHandler(err, targets) {
        ...
        targets = tmp;
        //simultaneously process subIncludes
        if (multiPointer[relation.modelTo.modelName] && targets) {
          tasks.push(function subIncludesTask(next) {
            relation.modelTo.multiPointer(targets, relation.modelTo.modelName, includefilter, multiPointer, next);
          });
        }
        ...
        if (subInclude && targets) {
          tasks.push(function subIncludesTask(next) {
            relation.modelTo.include(targets, subInclude, options, next, includefilter, multiPointer);
          });
        }
        //process each target object
        tasks.push(targetLinkingTask);
     ...
     * @param callback
     */
    function includeOneToOne(callback, includefilter, multiPointer) {
      relationName=relation.keyFrom;//因为Pointer多加了一个Pointer的后缀
      var targetIds = [];
      var objTargetIdMap = {};
      for (var i = 0; i < objs.length; i++) {
        var obj = objs[i];
      ...
      var modelToFilter = includefilter[ relation.modelTo.modelName];
      if (modelToFilter && modelToFilter.fields) {
        filter.fields = modelToFilter.fields;
      }
      filter.where[relation.keyTo] = {
        inq: uniq(targetIds)
      };
