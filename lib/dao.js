...
var multiInclusion = require('./multi.js')
...
DataAccessObject.find = function find(query, options, cb) {
  ...
  if (near) {
    var rprop = self.definition.rawProperties[near["key"]] || {};
    var supportsGeo = rprop.type == "GeoPoint" && rprop.index;
    if (supportsGeo) {
      // convert it
...
DataAccessObject.aggregate = function find(query, options, cb) {
  var connectionPromise = stillConnecting(this.getDataSource(), this, arguments);
  if (connectionPromise) {
    return connectionPromise;
  }
  ...
  try {
    for(var i= 0,len=query.length;i<len;i++){
      if(query[i]&&query[i].fields) continue;
      this._normalize(query[i]);
    }
  } catch (err) {
    process.nextTick(function () {
      cb(err);
  ...
  connector.aggregate(self.modelName, query, allCb);
  return cb.promise;
};
/*
 * Add 'include'
 ...
 * Add 'multiPointer'
 */
jutil.mixin(DataAccessObject, multiInclusion);
