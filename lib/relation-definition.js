...
RelationDefinition.hasMany = function hasMany(modelFrom, modelTo, params) {
  ...
  var fk = params.foreignKey || i8n.camelize(thisClassName + '(uz*R*id)', true);
  ...
  defineScope(modelFrom.prototype, params.through || modelTo, relationName, function () {
    var filter = {};
    filter.where = {};
    filter.where[fk] = String(this[pkName]);
...
RelationDefinition.removeHasManyOrBelongsTo = function hasMany(modelFrom, relationName){
  var pro =  modelFrom.prototype;
  var reg = new RegExp("_"+relationName+"$");
  Object.keys(pro).forEach(function(key){
    if(reg.test(key)){
      delete pro[key];
    }
  })
  delete modelFrom.relations[relationName];
  delete modelFrom.prototype[relationName];
}
